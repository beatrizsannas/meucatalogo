...
export default function EditarProdutoPage() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (upErr) { setUploadError('Erro ao enviar imagem.'); setIsUploading(false); return; }
            ...
            <main className="flex-1 p-8 overflow-auto">
                <div className="max-w-4xl">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-forest">Editar Produto</h1>
                            <p className="text-forest/50 text-sm mt-0.5">Atualizando item #{id.substring(0, 8).toUpperCase()}</p>
                        </div>
                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-6 items-start">
                                    ...
                                    <div className="flex-1 w-full space-y-3">
                                        <button type="button" onClick={() => fileInputRef.current?.click()}
                                            ...
                                            <Upload size={16} />Enviar do computador
                                        </button>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })}
                                            ...
                                            placeholder="Ex: algodão, importado, promoção" />
                                    </div>
                                </div>
                            </div>

                            {error && <p className="text-red-500 text-sm font-semibold mt-4 bg-red-50 p-3 rounded-xl">{error}</p>}

                            {/* Actions */}
                            <div className="mt-10 pt-6 border-t border-mint-dark flex items-center justify-end gap-3">
                                <Link href="/produtos" className="px-6 py-3 rounded-full text-sm font-semibold text-forest/60 hover:text-forest hover:bg-mint transition-colors">
                                    Cancelar
                                </Link>
                                <button type="submit" disabled={isSaving}
                                    className="flex items-center gap-2 px-8 py-3 rounded-full bg-lime text-forest text-sm font-bold hover:bg-lime-dark disabled:opacity-70 transition-colors shadow-sm active:scale-95">
